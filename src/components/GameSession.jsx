...
import { ArrowLeft, Save, Lock, AlertCircle, CheckCircle2, History as HistoryIcon, Trophy, Plus, TrendingDown, Crown, Moon, Unlock } from 'lucide-react';
...
const GameSession = ({ onNavigate }) => {
  ...
  const [currentRound, setCurrentRound] = useState({ bids: { Ayush: '', Mohit: '', Harsh: '' }, hands: { Ayush: '', Mohit: '', Harsh: '' } });
  const [phase, setPhase] = useState('bid'); // bid, play, hand
  const [popup, setPopup] = useState(null); // { type, countdown }
  ...
  const processRound = () => {
    ...
    setRounds([...rounds, { bids: { ...currentRound.bids }, hands: { ...currentRound.hands }, scores: roundScores }]);
        ...
        {leader && leader.score > 0 && (
          ...
          >
            <div className="bg-primary-500/10 border border-primary-500/20 rounded-2xl p-4 flex items-center space-x-4">
              ...
              <div>
                <p className="text-sm font-bold text-emerald-400">{leader.name} wins!</p>
      ...
      <div className="px-6 mb-12 grid grid-cols-3 gap-4">
        {playersWithDiffs.map(p => (
          ...
          >
            ...
            <p className={`text-xs font-bold uppercase tracking-widest mb-6 ${p.name === 'Ayush' ? 'text-emerald-500' : p.name === 'Harsh' ? 'text-blue-500' : 'text-orange-500'}`}>
              {p.name}
            </p>
      ...
      <div className="px-6">
        <div className="bg-dark-800/50 border border-white/5 rounded-[32px] overflow-hidden">
          ...
          <div className="relative">
                  ...
                  <div key={i} className="p-4 grid grid-cols-[30px_1fr_1fr_1fr_30px] gap-2 items-center hover:bg-white/[0.02]">
                    ...
                    {['Ayush', 'Harsh', 'Mohit'].map(name => (
                      <div key={name} className="text-center">
                        <span className="text-base font-bold text-slate-400">{r.scores[name]}</span>
                      </div>
                    ))}
            ...
            {/* Current Active Input Row Overlay */}
